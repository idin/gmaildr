"""
Test sender aggregation functionality.
"""
...
def test_aggregate_emails_by_sender():
    """
    Test basic sender aggregation functionality using real Gmail data.
    """
    gmail = Gmail()
    
    # Get real emails from Gmail
    emails = get_emails(gmail, n=20, include_text=False, include_metrics=False)
    
    if emails.empty:
        pytest.skip("No emails available to test sender aggregation")
    ...
    print(f"✅ Sender aggregation test passed! Processed {len(emails)} emails from {len(result)} unique senders")
